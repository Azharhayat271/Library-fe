...
const IssueBook = () => {
    ...
    const handleStudentSelect = (selectedStudent) => {
        ...
        setSearchValue('');
        form.setFieldsValue({ name: selectedStudent.name, regNo: selectedStudent.regNo });
    };

    const handleBookSelect = (selectedBook) => {
        setSelectedBook(selectedBook);
        setBookSearchValue('');
        form.setFieldsValue({ title: selectedBook.title, ISBN: selectedBook.ISBN });
    };
    ...
    const handleConfirm = () => {
        if (selectedStudent && selectedBook) {
            ...
            axios.post('http://localhost:5000/bookissue/bookissue', requestData)
                .then((response) => {
            ...
            console.error('Please select both a student and a book before confirming.');
        }
    };
                    ...
                    <ul className="space-y-2">
                        {foundStudents.map((student) => (
                            ...
                            >
                                ...
                                <span className="text-gray-500 text-sm ml-2">({student.regNo})</span>
                            </li>
                        ))}
                    </ul>
                )}
                    ...
                    <ul className="space-y-2">
                        {foundBooks.map((book) => (
                            ...
                            >
                                ...
                                <span className="text-gray-500 text-sm ml-2">({book.ISBN})</span>
                            </li>
                        ))}
                    </ul>
                )}
